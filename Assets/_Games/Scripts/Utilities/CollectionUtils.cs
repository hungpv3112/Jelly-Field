...
public static class CollectionUtils
{
    ...
    public static int GetIndexByRatioInArray(List<float> listRatio, List<int> listIndexIgnore)
    {
        if(listIndexIgnore == null || listIndexIgnore.Count == 0)
        {
            return GetIndexByRatioInArray(listRatio);
        }

        if (listRatio.Count == listIndexIgnore.Count) return -1;

        var listIndex = new List<int>();
        for (int i = listRatio.Count - 1; i >= 0; i--)
        {
    ...
    {
        return GetIndexByRatioInArray(array.ToList(), listIndexIgnore);
    }
